...
<body>
  <main class="shell">
    ...
    <section class="app" aria-label="Диалог с Домашним дворецким">
      ...
      <form id="chat"><textarea id="input" maxlength="2000" rows="2" aria-label="Сообщение" placeholder="Дайте модели вопрос или задачу"></textarea><button id="send" type="submit">Отправить</button><p class="hint"><label class="mode"><input id="direct-mode" type="checkbox" checked> Естественный режим с проверкой Home Assistant</label>Enter — отправить, Shift+Enter — новая строка. Для точного штатного отчёта HA снимите флажок.</p></form>
    </section>
  </main>
  <script>
