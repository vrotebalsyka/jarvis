...
def run(manifest_path: Path, inventory_path: Path) -> dict[str, Any]:
    ...
    try:
        for row in manifest:
            ...
            if result is not None and result.frame.kind == "clarification":
                ...
                model_clarification += result.frame.selector_used
            if plan is not None:
            ...
            if row.get("expected_action_intent", True) is False:
                false_action_intent += result is not None and result.action_plan is not None
    ...
    status = "pass" if (
        ...
        == missed_expected_plan == wrong_action == false_action_intent
        == ha_post_attempts == ha_service_path_attempts == service_calls
        == model_prompt_leaks == 0
    ...
    return {
        ...
        "false_action_intent": false_action_intent,
        ...
        "ha_read_attempts": ha_read_attempts, "service_calls": service_calls,
